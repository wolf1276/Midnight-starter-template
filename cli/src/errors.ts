...
/*
 * A reusable, typed error system for the CLI.
 *
 * Every failure that can reach a user should end up as a `CLIError` (or subclass) by the
 * time it's printed. `classifyError()` maps whatever raw error came out of Docker, Node,
 * npm, the Compact toolchain, the SDK, or the filesystem into one of these, so the terminal
 * never shows a raw stack trace unless the user asked for one with --verbose.
 */
...
export interface CLIErrorDetails {
  /** Short title shown after the ❌, e.g. "Port Already In Use". */
  title: string;
  /** Plain-English explanation of what happened. */
  whatHappened: string;
  /** Exact commands/steps to fix it. */
  howToFix: string;
  /** Extra detail only ever shown with --verbose (appended before the raw stack). */
  moreInfo?: string;
  /** Process exit code. Defaults to 1. */
  exitCode?: number;
}

/** Base class for every user-facing CLI failure. Never throw a raw Error — throw one of these. */
export class CLIError extends Error {
  ...
  constructor(details: CLIErrorDetails, options?: { cause?: unknown }) {
...
export function formatCliError(err: CLIError, verbose: boolean): string[] {
  const lines: string[] = [];
  lines.push('');
  lines.push(red(bold(`❌ ${err.title}`)));
  lines.push('');
  lines.push(bold('What happened'));
  lines.push('-------------');
  lines.push(err.whatHappened);
  lines.push('');
  lines.push(bold('How to fix it'));
  lines.push('-------------');
  lines.push(err.howToFix);
  if (verbose) {
    lines.push('');
    lines.push(bold('More information'));
    lines.push('----------------');
    if (err.moreInfo) lines.push(err.moreInfo);
    if (err.cause)
      lines.push(dim(err.cause instanceof Error ? (err.cause.stack ?? err.cause.message) : JSON.stringify(err.cause)));
    if (err.stack) lines.push(dim(err.stack));
  } else {
    lines.push('');
    lines.push(dim(`Re-run with ${cyan('--verbose')} for full diagnostic output.`));
  }
  lines.push('');
  return lines;
}

export function printCliError(err: CLIError, verbose: boolean): void {
...
const SERVICE_BY_PORT: Record<number, string> = Object.fromEntries(
  Object.entries(versions.ports).map(([port, service]) => [Number(port), service]),
);
 ...
 * error, filesystem error, etc.) into a typed `CLIError` with a plain-English explanation
 * and exact next steps. `retryCommand` is the command the user should re-run once fixed.
 */
export function classifyError(e: unknown, retryCommand?: string): CLIError {
    ...
    return new DockerError(
      {
        ...
        howToFix:
          'Start Docker Desktop (or run `sudo systemctl start docker` on Linux), wait for it to finish starting, then retry.',
      },
      { cause },
    );
  }
    ...
    return new DockerError(
      {
        ...
        howToFix:
          'Remove the conflicting container(s) and retry:\n\n  npm run blockchain:reset\n  npm run blockchain:start',
      },
      { cause },
    );
  }
    ...
    return new DockerError(
      {
        ...
        howToFix:
          'Reset volumes and retry (this clears local chain/indexer data):\n\n  npm run docker:reset\n  npm run blockchain:start',
      },
      { cause },
    );
  }
      ...
      return new NpmError(
        {
          ...
          howToFix:
            "Don't use sudo with npm. Fix ownership of your npm cache/project directory:\n\n  sudo chown -R $(whoami) ~/.npm .\n\nthen re-run:\n\n  npm install",
        },
        { cause },
      );
    }
    ...
    return new CompactError(
      {
        ...
        howToFix:
          "Install the Compact CLI, then verify it: \n\n  curl --proto '=https' --tlsv1.2 -LsSf https://github.com/midnightntwrk/compact/releases/latest/download/compact-installer.sh | sh\n  compact --version",
      },
      { cause },
    );
  }
    ...
    return new CompactError(
      {
        ...
        howToFix:
          'Re-run the build with --verbose to see the full verifier output, then fix the contract and rebuild:\n\n  npm run build:contract',
      },
      { cause },
    );
  }
    ...
    return new DeploymentError(
      {
        ...
        howToFix:
          'Check deployment.json for the existing address, or reset the wallet to deploy fresh:\n\n  npm run wallet:reset',
      },
      { cause },
    );
  }
    ...
    return new NetworkError(
      {
        ...
        howToFix:
          'If you are behind a corporate proxy/VPN, configure NODE_EXTRA_CA_CERTS, or check the system clock, then retry.',
      },
      { cause },
    );
  }
...
/**
 * Installs a top-level handler for a launcher script: runs `fn`, and on any failure prints
 * a structured `CLIError` (classifying raw errors first) instead of letting a stack trace
...
export async function runMain(
  fn: () => Promise<void>,
  opts: { verbose: boolean; retryCommand?: string; onFatal?: (err: CLIError) => Promise<void> },
): Promise<void> {
  const fail = async (e: unknown): Promise<never> => {
    const err = classifyError(e, opts.retryCommand);
    printCliError(err, opts.verbose);
    if (opts.onFatal) await opts.onFatal(err).catch(() => {});
